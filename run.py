...
def get_all_keypoint(frame, cfg) -> Target:
    # get head coordinate
    boxes_array = frame.keypoints.xy.to(f'cuda:{cfg.AI_device}')
    return boxes_array[0, :, :].cpu().numpy()
...
def init():
    ...
    try:
        model = YOLO(f'models/{cfg.AI_model_path}', task='pose')
    ...
    while True:
        ...
        #pose estimation
            ...
            for frame in result:
                if len(frame.boxes):
                    if app_pause == 0:
                        target = get_all_keypoint(frame, cfg)
                        if target is not None:
                            head_x, head_y = target[3, 0], target[3, 1]
                            # mouse_worker.queue.put((target.x, target.y, target.w, target.h))
                            if cfg.show_window or cfg.show_overlay_detector:
                                screen_x_center = cfg.detection_window_width / 2
                                screen_y_center = cfg.detection_window_height / 2
                                ...
                                if cfg.show_target_line and cfg.show_window:
                                    if not math.isnan(head_x) and not math.isnan(head_y) and head_x != 0 and head_y != 0 :
                                        draw_target_line(annotated_frame, screen_x_center, screen_y_center, head_x, head_y)
                            ...
                            if cfg.show_overlay_detector:
                                if cfg.show_overlay_boxes:
                                    ...
                                    # draw points
                                    for x,y in target:
                                        overlay.canvas.create_oval(x - 2, y - 2, x + 2, y + 2, fill='red')

                                    # connect points
                                    for i, sk in enumerate(skeleton):
                                        pos1 = (float(target[(sk[0] - 1), 0]), float(target[(sk[0] - 1), 1]))
                                        pos2 = (float(target[(sk[1] - 1), 0]), float(target[(sk[1] - 1), 1]))
                                        if pos1[0] % image.shape[1] == 0 or pos1[1] % image.shape[0] == 0 or pos1[0] < 0 or pos1[1] < 0:
                                            continue
                                        if pos2[0] % image.shape[1] == 0 or pos2[1] % image.shape[0] == 0 or pos2[0] < 0 or pos2[1] < 0:
                                            continue
                                        overlay.canvas.create_line(pos1[0], pos1[1], pos2[0],pos2[1], width=2, fill='green')
                    else: pass
